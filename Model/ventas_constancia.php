...
class ventas_constancia
{
	...
	public function Listar()
	{
		try
		{
			...
			$stm = $this->pdo->prepare("SELECT * FROM ventas_constancia");
	...
	public function Registrar(ventas_constancia $data)
	{
		try
		{
			//Sentencia SQL.
			$sql = "insert into ventas_constancia values (?,?);";

			$this->pdo->prepare($sql)
		     ->execute(
				array(
                    $data->numero_factura,
                    $data->numero_constancia
                )
			);
		} catch (Exception $e)
		{
